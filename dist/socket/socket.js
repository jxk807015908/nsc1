  ...
  io.sockets.on('connection', function (socket) {
                    ...
                    if (toUser) {
                      searchUser.searchUser({userId: userId}, result2 => {
                        if (result2 === 'error') {
                          ...
                          io.sockets.to(toUser.socketId).emit('friendLoginRemand', {
                            userId: userId,
                  ...
                  if (toUser) {
                    searchUser.searchUser({userId: userId}, result2 => {
                      if (result2 === 'error') {
                        ...
                        io.sockets.to(toUser.socketId).emit('friendDisconnectRemand', {
                          userId: userId,
                          name: obj.F_Name || result2[0].U_NickName || userId
                        });
                        io.sockets.to(toUser.socketId).emit('refreshFriend');
    ...
    socket.on('remandFriend', function (obj) {
      if (obj.groupId) {
        ...
        selectUserGroupsToUser2.selectUserGroupsToUser2({groupId: obj.groupId}, (result) => {
          if (result === 'error') {
            ...
            toUsers.forEach(item => {
              ...
              io.sockets.to(item.socketId).emit('getNewRemind', obj);
            })
          }
        });
      } else {
        ...
        if (toUser.length === 1) {
          ...
          io.sockets.to(toUser[0].socketId).emit('getMessageToHead', obj);
          io.sockets.to(toUser[0].socketId).emit('getNewRemind', obj);
        }
      }
    });
    //
    ...
    socket.on('refreshFriend', function (obj) {
      let toUser = users.filter(item => item.userId === obj.toId);
      if (toUser.length === 1) {
        console.log("已发送提醒");
        io.sockets.to(toUser[0].socketId).emit('refreshFriend', obj);
      }
    });
    socket.on('refreshGroup', function (obj) {
      let toUser = users.filter(item => item.userId === obj.toId);
      if (toUser.length === 1) {
        console.log("已发送提醒");
        io.sockets.to(toUser[0].socketId).emit('refreshGroup', obj);
      }
    });
    socket.on('sendRemind', function (obj) {
      //console.log('---------------------')
      //socket.broadcast.emit('newImg', socket.nickname, imgData, color);
      console.log('正在发提醒');
      if(obj.type===2){
        selectUserGroupsToUser3.selectUserGroupsToUser3({
          groupId: obj.toId,
          authority:3
        }, (result) => {
          if (result === 'error') {
            ...
            let toUsers = users.filter(item => members.includes(item.userId));
            // console.log('-------------------------------------');
            // console.log(users);
            // console.log(toUsers);
            // console.log(members);
            toUsers.forEach(item => {
              console.log('给' + item.userId + '发送消息推送');
              io.sockets.to(item.socketId).emit('RefreshRemind', obj);
              io.sockets.to(item.socketId).emit('getNewRemind', obj);
            })
          }
        });
        ...
        if (toUser.length === 1) {
          console.log("已发送提醒");
          io.sockets.to(toUser[0].socketId).emit('getNewRemind', obj);
          io.sockets.to(toUser[0].socketId).emit('RefreshRemind', obj);
        }
      }
    });
    // socket.on('getMessage', function (obj) {
    //   //console.log('---------------------')
    //   //socket.broadcast.emit('newImg', socket.nickname, imgData, color);
    //   // console.log('正在发提醒');
    //   updateUserMessages2.updateUserMessages2({
    //     update:{
    //       tipNum:0
    //     },
    //     select:{
    ...
    //       toId:obj.userId,
    //       messageType:'1',
    //       remark:obj.type==1?'friend':'group'
    //     }
    //   },(result)=>{
    //     if(result==="error"){
    //     }else{
