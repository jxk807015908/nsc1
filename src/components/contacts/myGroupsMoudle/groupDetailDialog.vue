<template>
  <div class="groupDetailDialog">
    <dialogs :dialogFlag.sync="dialogFlag" :openTitle="'ID:'+openData.groupId" :isDefaultBtn="false" @close="close" @open="open">
      <div slot="dialogContent">
        ...
        <div class="group-nick" v-if="openData.members&&openData.members&&openData.members.some(obj =>obj.memberId === $store.state.userId)">
          <el-row>
        ...
        <el-popover
          ...
          <div v-popover:friendList v-if="openData.members&&openData.members.some(obj =>obj.memberId === $store.state.userId)"><i class="el-icon-plus"></i><span>邀请新成员</span></div>
          <div class="table-box">
            <!--<table>-->
            ...
            <ul>
              <li v-for="obj in openData.members" :key="obj.memberId">
                ...
                <div class="operation">
                  <a v-if="openData.adminId === $store.state.userId && openData.adminId!== obj.memberId" href="javascript:;" title="移交群主" @click="changeAdmin(obj)">移交</a>
                  <a v-if="$store.state.userId=== obj.memberId" href="javascript:;" title="退出该群" @click="quitGroup">退出</a>
                  <a v-if="openData.adminId === $store.state.userId && openData.adminId!== obj.memberId" href="javascript:;" title="请离该群" @click="moveOutMember(obj)">请离</a>
                </div>
              </li>
            </ul>
          </div>
        ...
        <div class="group-intro">
          <el-row>
            ...
            <el-col :span="23" :offset="1">
              ...
              <el-input v-if="$store.state.userId===openData.adminId" v-model="params.groupIntro" type="textarea" placeholder="请输入群介绍"></el-input>
...
<script>
  ...
  export default {
    ...
    methods:{
      open(){
        this.fileList=[{name:'groupIcon',url:`http://${this.$store.state.statisFileIp}${this.openData.groupIcon}`}];
      ...
      changeAdmin(member){
        this.$http.post('/changeAdmin.do',{groupId:this.openData.groupId,adminId:member.memberId,userId:this.$store.state.userId}).then(res=>{
          if (res.data.success) {
      ...
      dismissGroup(){
        this.$http.post('/dismissGroup.do',{groupId:this.openData.groupId,userId:this.$store.state.userId}).then(res=>{
          if (res.data.success) {
      ...
      select(index,indexPath){
        ...
        this.$http.post('/inviteGroup.do',{toId:index,fromId:this.$store.state.userId,remark:remark,fromName:this.$store.state.nickName}).then(res=>{
          if(res.data.success){
